...
func TestGetUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mocks: func(df *depfields) {
				...
				df.persistMock.On("GetUser", mock.Anything, mock.Anything).Return(mockUser, nil)
			},
		...
		},
		{
			...
			mocks: func(df *depfields) {
				df.persistMock.On("GetUser", mock.Anything, mock.Anything).Return(nil, fmt.Errorf("db error: %w", errors.New("user not found")))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx.Request = httptest.NewRequest(http.MethodGet, "/users/"+mockUUID.String(), nil)
