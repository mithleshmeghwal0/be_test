...
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mocks: func(df *depfields) {
				...
				df.persistMock.On("CreateUser", mock.Anything, mock.Anything).Return(mockUser, nil)
			},
		...
		{
			name: "persist.CreateUser() error",
			args: args{
				reqbody: []byte(`{"name":"test","email":"test@test.com"}`),
				log:     logger.New(),
			},
			mocks: func(df *depfields) {
				df.persistMock.On("CreateUser", mock.Anything, mock.Anything).Return(nil, fmt.Errorf("db error: %w", errors.New("internal error")))
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx.Request = httptest.NewRequest(http.MethodPost, "/users", bytes.NewReader(tt.args.reqbody))
